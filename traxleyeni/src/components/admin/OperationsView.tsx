...
import { FiTool, FiArrowRight, FiCheckCircle, FiClock, FiXCircle } from 'react-icons/fi';
...
export default function OperationsView({ loads }: { loads: Load[] }) {
    ...
    const handleCancelLoad = async (id: string) => {
        if (!confirm("Bu kiralama talebini iptal etmek istediğinize emin misiniz? Tedarikçi atanmışsa işlem geri alınamaz!")) return;
        try {
    ...
    const getStatusConfig = (status: Load['status']) => {
        switch (status) {
            case 'active': return { color: 'bg-blue-100 text-blue-700', label: 'AKTİF / SAHADA', icon: <FiTool className="inline mr-1" /> };
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h2 className="text-2xl font-bold flex items-center gap-2"><FiTool className="text-blue-500" /> Operasyon Merkezi</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loads.length === 0 ? (
                    <div className="col-span-full p-12 bg-white dark:bg-[#0A0A0A] border border-dashed rounded-3xl text-center text-gray-500">Sistemde aktif kiralama talebi bulunmuyor.</div>
                ) : (
                    loads.map((load) => {
                            ...
                            <div key={load.id} className="bg-white dark:bg-[#0A0A0A] p-6 rounded-3xl border border-gray-200 dark:border-white/10 flex flex-col justify-between shadow-sm">
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        ...
                                        <h3 className="font-bold text-lg text-gray-900 dark:text-white">Talep #{load.id.slice(0, 8)}</h3>
                                    </div>
                                ...
                                <div className="space-y-4 mb-8 bg-gray-50 dark:bg-white/5 p-4 rounded-2xl">
                                    <div className="flex items-start gap-3 text-sm">
                                        ...
                                        <div><span className="text-xs text-gray-400 block mb-0.5">Çıkış Noktası</span><span className="font-medium">{load.pickup}</span></div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3 text-sm">
                                        ...
                                        <div><span className="text-xs text-gray-400 block mb-0.5">Şantiye Teslim</span><span className="font-medium">{load.delivery}</span></div>
                                    </div>
                                </div>
