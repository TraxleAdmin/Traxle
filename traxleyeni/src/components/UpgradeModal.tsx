...
export default function UpgradeModal({ isOpen, onClose, targetPlan = 'pro' }: UpgradeModalProps) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      await setDoc(userRef, {
        walletBalance: increment(-plan.price),
  ...
  let Icon = FiStar;
  let iconColor = 'from-purple-500 to-indigo-600'; // Premium paketin varsayılan rengi ve ikonu

  // 🔥 Tip (Type) Hatası Giderildi: 'starter' yerine 'pro' kullanılıyor
  if (plan.id === 'pro') { Icon = FiZap; iconColor = 'from-blue-400 to-cyan-500'; }
