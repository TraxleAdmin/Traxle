...
// Tip Tanımlamaları (Sayfalar arası veri uyumu için)
...
// Filtre Butonu (Sadece bu modüle özel olduğu için burada tutuyoruz)
    ...
    return (
        <button 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all border ${
                active 
                    ? (isDanger ? 'bg-red-600 text-white border-red-600' : 'bg-blue-600 text-white border-blue-600') 
...
export default function UsersModule({ users, openDetail, exportCSV }: UsersModuleProps) {
    ...
    const filtered = users.filter((u) => {
        ...
        if(filterType === 'drivers' && u.role !== 'driver') return false;
        ...
        <div className="flex flex-col h-full gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                    ...
                    <FilterButton label="Sürücüler" active={filterType === 'drivers'} onClick={() => setFilterType('drivers')} />
                ...
                <div className="flex gap-2 shrink-0">
                    <button 
                        onClick={exportCSV} 
                        className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-green-500/20 transition-all active:scale-95"
                    >
                    ...
                    <div className="relative w-full md:w-64">
                        ...
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"
                        />
                    </div>
                </div>
            </div>
                                ...
                                filtered.map((user) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 font-medium flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shadow-inner">
                                                {user.name?.charAt(0).toUpperCase() || '?'}
                                            </div>
                                            <div>
                                                <p className="text-gray-900 dark:text-white group-hover:text-blue-500 transition-colors">{user.name}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            {user.isVerified ? (
                                                <span className="inline-flex items-center gap-1 text-green-600 bg-green-100 dark:bg-green-900/20 px-2 py-1 rounded text-xs font-bold">
                                                    Onaylı
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1 text-orange-600 bg-orange-100 dark:bg-orange-900/20 px-2 py-1 rounded text-xs font-bold">
                                                    Bekliyor
                                                </span>
                                            )}
                                        </td>
