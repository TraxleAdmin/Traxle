...
export default function LoginPage() {
  ...
  // 'email' yerine 'identifier' (kimlik) kullanıyoruz
  ...
  const handleSocialLogin = async (providerName: 'google' | 'apple') => {
    setIsLoading(true);
    setGlobalError(null);
    const res = await socialLogin(providerName, 'shipper'); 
    if (res.success) {
      router.push('/panel');
    } else {
  ...
  const validateForm = () => {
    ...
    // 1. E-posta mı Telefon mu kontrolü
    if (!inputVal) {
        ...
        // TELEFON KONTROLÜ (Boşlukları ve karakterleri temizle)
        const cleanPhone = inputVal.replace(/\D/g, ''); 
        // Türkiye standardı: 10 hane (5XX...) veya 11 hane (05XX...)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Backend'e 'email' parametresi olarak identifier'ı gönderiyoruz.
    // Backend'in bunu algılaması veya burada ayrıştırılması gerekebilir.
    // Şimdilik string'i olduğu gibi iletiyoruz.
  ...
  return (
    <div className="min-h-screen flex bg-white dark:bg-[#050814] transition-colors duration-500">
      ...
      {/* --- SOL TARAF: GÖRSEL (LIGHT/DARK UYUMLU) --- */}
        ...
        <div className="relative z-10 px-12 max-w-2xl text-center lg:text-left">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                ...
                <p className="text-lg leading-relaxed text-gray-600 dark:text-gray-400 transition-colors duration-500">
                    Traxle OS paneline giriş yaparak filonuzu, yüklerinizi ve finansal durumunuzu anlık yönetmeye devam edin.
                </p>
            </motion.div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative">
         <div className="w-full max-w-md space-y-8">
            ...
            {/* 🔥 SOSYAL GİRİŞ BUTONLARI BURAYA EKLENDİ */}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
               <div>
                   {/* TEK INPUT: Hem E-posta Hem Telefon */}
                   <InputGroup 
                     icon={<FiUser />} 
                     type="text" 
                     name="identifier" 
            ...
            <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-8">
               Hesabınız yok mu? <Link href="/kayit-ol" className="text-blue-600 dark:text-blue-400 font-bold hover:underline ml-1">Hemen Kayıt Ol</Link>
            </p>
         </div>
      </div>
    </div>
