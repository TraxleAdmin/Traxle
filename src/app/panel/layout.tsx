...
import { 
  FiSettings, FiLogOut, FiMenu, FiX, FiUser, FiAlertTriangle, FiCheckCircle,
  FiStar, FiShield, FiSmartphone 
} from 'react-icons/fi';
...
function UserBadge({ plan, role }: { plan: string, role: string }) {
  ...
  const icon = role === 'admin' ? <FiShield /> : plan === 'pro' ? <FiStar /> : <FiUser />;
  const text = role === 'admin' ? "YÖNETİCİ" : plan === 'pro' ? "PRO ÜYE" : "STANDART";
...
export default function PanelLayout({ children }: { children: React.ReactNode }) {
      ...
      if (!currentUser) {
        ...
        const unsubDoc = onSnapshot(doc(db, "users", currentUser.uid), (docSnap) => {
             if (docSnap.exists()) {
                ...
                // 🔥 AKILLI YÖNLENDİRME (Ana panele girildiğinde yetkiye göre dağıt)
                if (pathname === '/panel') {
        ...
        // Skeleton efektini hissettirmek için ufak bir gecikme
  ...
  // 🔥 YENİ: SKELETON LOADING (Milyar Dolarlık Silikon Vadisi Standardı)
  if (loading) {
    return (
      <div className="flex h-screen bg-[#F3F4F6] dark:bg-[#050814] w-full overflow-hidden">
        ...
        <div className="w-[280px] border-r border-gray-200 dark:border-white/5 p-6 hidden lg:flex flex-col gap-8 bg-white dark:bg-[#0B0F19]">
           ...
           <div className="space-y-4 mt-4">
              {[1,2,3,4].map(i => <div key={i} className="w-full h-12 bg-gray-200 dark:bg-white/5 animate-pulse rounded-xl"></div>)}
           </div>
        </div>
        {/* Main Content İskeleti */}
  ...
  // 🔥 ÇİFT MENÜ KALKANI: Eğer Admin veya Staff sayfasındaysak standart layout'u çizme!
  ...
  if (isSpecialRoute) {
      return (
          <div className="h-screen bg-[#050814] text-white overflow-hidden flex flex-col">
              {/* E-posta uyarısını bu sayfalarda da en üstte tutuyoruz */}
  ...
  const menuConfig = {
    shipper: [
      ...
      { icon: <FiSettings />, label: 'Profil & Güvenlik', href: '/panel/ayarlar' },
    ],
    driver: [
      { icon: <FiSmartphone />, label: 'Mobil Uygulama', href: '/panel' },
      { icon: <FiSettings />, label: 'Profil & Güvenlik', href: '/panel/ayarlar' },
  ...
  // Eğer admin veya staff kazara standart url'de kalırsa hata vermemesi için
  const currentMenu = menuConfig[userData?.role as keyof typeof menuConfig] || menuConfig.shipper;
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F6] dark:bg-[#050814] text-gray-900 dark:text-gray-100 overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      {userData?.role === 'driver' && <LocationTracker />}
          ...
          <div className="p-6 pb-2">
              <div className="bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/5 flex items-center gap-4 group hover:border-blue-500/30 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-sm truncate">{userData?.name || 'Kullanıcı'}</h4>
                    <UserBadge plan={userData?.subscriptionPlan} role={userData?.role} />
                </div>
              </div>
          </div>
           ...
           <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                 <h2 className="text-lg font-bold text-gray-900 dark:text-white capitalize leading-tight">
                    {pathname === '/panel' ? 'Mobil Uygulama' : pathname?.split('/').pop()?.replace(/-/g, ' ')}
                 </h2>
                 <p className="text-xs text-gray-500 hidden sm:block">Traxle Kullanıcı Paneli</p>
              </div>
           </div>
