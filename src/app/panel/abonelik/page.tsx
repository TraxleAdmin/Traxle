...
export default function SubscriptionPage() {
  ...
  const getPlanIcon = (id: string) => {
    switch (id) {
      case 'pro': return <FiZap className="w-6 h-6" />;
      case 'premium': return <FiStar className="w-6 h-6" />;
    ...
    <div className="max-w-7xl mx-auto pb-20 px-4 md:px-0">
      ...
      <div className="text-center mb-16 pt-8 space-y-4">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-gray-500 dark:text-gray-400 max-w-2xl mx-auto text-lg">
          Daha fazla iş makinesi ilanı, öne çıkma (doping) ayrıcalıkları ve premium destek ile kazancınızı artırın. İstediğiniz zaman iptal edebilirsiniz.
        </motion.p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 relative items-start">
        ...
        {Object.values(PLANS).map((plan, index) => {
          ...
          const isHighlighted = plan.ui.popular; // plans.ts'den gelen 'popular' değeri ile parlatma yapılır
          ...
          return (
            ...
            >
              {/* Popüler Rozeti */}
              {isHighlighted && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-bold px-4 py-1.5 rounded-full shadow-lg tracking-wider uppercase flex items-center gap-1 whitespace-nowrap">
              ...
              <div className="mb-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 text-2xl shadow-lg transition-transform
                  ...
                  ${plan.id === 'pro' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600' : ''}
                  ${plan.id === 'premium' ? 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white' : ''}
                  ${plan.id === 'enterprise' ? 'bg-gray-900 dark:bg-white/10 text-white' : ''}
                `}>
                  {getPlanIcon(plan.id)}
                </div>
              ...
              <div className="flex-1 space-y-4 mb-8">
                {plan.displayFeatures.map((feature, i) => (
                  <div key={i} className="flex items-start gap-3 text-sm group">
                    {feature.included ? (
                      <div className={`mt-0.5 p-0.5 rounded-full shrink-0 ${isHighlighted ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-400'}`}>
                        <FiCheck size={10} />
                      </div>
                    ) : (
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
        <div className="p-6 rounded-3xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5">
          ...
          <p className="text-xs text-gray-500">Tüm ödemeleriniz 256-bit SSL sertifikası ve BDDK lisanslı güvenli ödeme altyapısı ile güvence altındadır.</p>
        </div>
        <div className="p-6 rounded-3xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5">
        ...
        <div className="p-6 rounded-3xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5">
          ...
          <p className="text-xs text-gray-500">Pro ve Premium üyelerimiz için öncelikli WhatsApp ve telefon desteği mevcuttur.</p>
        </div>
      </div>
